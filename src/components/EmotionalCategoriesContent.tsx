...
export default function EmotionalCategoriesContent() {
    const { categories, isFetching, refetch, deleteCategory } = useEmotionalCategories()

    const [selectedCategory, setSelectedCategory] = useState<EmotionalCategory>()
    const [createCategoryVisible, setCreateCategoryVisible] = useState<boolean>(false)
    const [updateCategoryVisible, setUpdateCategoryVisible] = useState<boolean>(false)
    const [deleteCategoryVisible, setDeleteCategoryVisible] = useState<boolean>(false)
    const [categoryToDelete, setCategoryToDelete] = useState<EmotionalCategory | null>(null)
            ...
            <>
                ...
                <div className="mb-8">
                    <div className="flex items-center space-x-3 mb-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Emotional Categories</h1>
                            <p className="text-gray-600">Manage emotional categories and their sub-emotions</p>
                        </div>
                    </div>
                </div>
                <CategoriesSkeleton />
        ...
        <>
            ...
            <div className="mb-8">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Emotional Categories</h1>
                            <p className="text-gray-600">Manage emotional categories and their sub-emotions for AI therapy</p>
                        </div>
                    </div>
                    <button
                            ...
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">{category.emotions.length} emotions</p>
                                    </div>
                                </div>
                                ...
                                <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-4 mb-4">
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div className="flex-1">
                                            <h4 className="text-sm font-semibold text-gray-900 mb-1">AI Prompt</h4>
                                            <p className="text-sm text-gray-700 leading-relaxed">{category.prompt}</p>
                                        </div>
                                    </div>
                                </div>
                            )}
                                    ...
                                    {category.emotions.map((emotion, index) => (
                                        <div key={index} className="bg-white rounded-lg p-4 border border-gray-200">
                                            <div className="flex items-start justify-between">
                                                <div className="flex-1">
                                                    <h5 className="text-sm font-semibold text-gray-900 mb-2">{emotion.name}</h5>
                                                    {emotion.prompt && (
                                                        <p className="text-xs text-gray-600 leading-relaxed">{emotion.prompt}</p>
                                                    )}
                                                </div>
                                            </div>
                                        </div>
            ...
            {categories?.length === 0 && (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-6">Get started by creating your first emotional category.</p>
                    <button
                        onClick={() => setCreateCategoryVisible(true)}
                        className="inline-flex items-center px-6 py-3 border border-transparent text-sm font-medium rounded-xl text-white bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-all duration-200 transform hover:scale-105 shadow-lg"
                    >
